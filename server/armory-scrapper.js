...
module.exports = scrapper = function scrapArmory(character, realm, callback, hash) {
  (async () => {
    ...
    const backgroundImageUrl = await page.evaluate((el) => window.getComputedStyle(el).backgroundImage, await page.$('.ProfilePage > .Pane-bg'));
    const backgroundImageCleaned = Buffer.from(backgroundImageUrl.match(/url\("(.*)"/)[1].replace(/^data:image\/png;base64,/, ""), 'base64')
